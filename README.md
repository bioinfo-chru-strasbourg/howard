...
This process tool include a chunking process to handle large files by dividing them into smaller, more manageable pieces (chunks), by specifying chunk size and partitioning scheme. This approach allows each chunk to be processed independently, reducing memory usage, disk usage (duckDB swap), and can help prevent crashes or slowdowns due to memory constraints. Once all chunks are processed, the results are merged to produce the final output.
