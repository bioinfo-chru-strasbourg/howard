    ...
    if "generate_param" in args and args.generate_param:
        ...
        if args.generate_param_releases:
            generate_param_releases = [
                value for value in args.generate_param_releases.split(",")
            ]
        else:
            generate_param_releases = []
        ...
        if args.generate_param_formats:
            generate_param_formats = [
                value for value in args.generate_param_formats.split(",")
            ]
        else:
            generate_param_formats = []
        ...
        for databases_param_stat in databases_param_stats:
            if not isinstance(databases_param_stats[databases_param_stat], dict):
                log.info(
                    f"{databases_param_stat}: {databases_param_stats[databases_param_stat]}"
                )
            else:
                log.info(f"{databases_param_stat}")
                for databases_param_substat in databases_param_stats[
                    databases_param_stat
                ]:
                    if not isinstance(
                        databases_param_stats[databases_param_stat][
                            databases_param_substat
                        ],
                        dict,
                    ):
                        log.info(
                            f"   {databases_param_substat}: {databases_param_stats[databases_param_stat][databases_param_substat]}"
                        )
                    else:
                        log.info(f"   {databases_param_substat}")
                        for databases_param_subsubstat in databases_param_stats[
                            databases_param_stat
                        ][databases_param_substat]:
                            log.info(
                                f"      {databases_param_subsubstat}: {databases_param_stats[databases_param_stat][databases_param_substat][databases_param_subsubstat]}"
                            )
    ...
    with pd.HDFStore(output_file, mode="w") as store:
        for i in range(0, num_rows, chunk_size):
            table = parquet_reader.read_row_group(
                0, columns=columns, skip_rows=i, num_rows=chunk_size
            ).to_pandas()
            table = table.apply(
                lambda row: ";".join(
                    [f"{col}={row[col]}" for col in columns if pd.notnull(row[col])]
                ),
                axis=1,
            )
...
def databases_infos(
    database_folders: list = [],
    database_folder_releases: list = ["current"],
    assembly: str = "hg19",
    database_formats: list = None,
    config: dict = {},
) -> dict:
    """
    ...
    if not database_folders:
        config_database_folders = (
            config.get("folders", {}).get("databases", {}).get("annotations", [])
        )
        for config_database_folder in config_database_folders:
            config_database_folder_dirname = os.path.dirname(
                re.sub(r"/*$", "", config_database_folder)
            )
            log.debug(f"config_database_folder: {config_database_folder}")
            log.debug(
                f"   config_database_folder_dirname: {config_database_folder_dirname}"
            )
    ...
    if not database_folders:
        ...
        for database_folder in database_folders_list:
            if os.path.isdir(os.path.join(default_database_folder, database_folder)):
                database_folders.append(
                    os.path.join(default_database_folder, database_folder)
                )
        ...
        for root, dirs, files in os.walk(f"{database_folder}", followlinks=True):
            ...
            if level == 2:
                ...
                if os.path.isdir(root):
                    ...
                    if (
                        not database_folder_releases
                        or database_release in database_folder_releases
                    ) and (database_assembly == assembly):

                        # List files within the folder
                        list_of_files = glob.glob(
                            os.path.join(root, "**"), recursive=True
                        )
                        ...
                        for database in list_of_files:
                            ...
                            if os.path.exists(database):
                                ...
                                database_format = vcfdata_obj.get_format(
                                    database=database
                                )
                                ...
                                if (
                                    os.path.isfile(f"{database}.hdr")
                                    or (
                                        os.path.isfile(database)
                                        and database.endswith(
                                            ("vcf", "vcf.gz", "vcf.bgz")
                                        )
                                    )
                                ) and (
                                    not database_formats
                                    or database_format in database_formats
                                ):

                                    # Database subfolder (allow files in subfolder, such as database versions)
                                    database_subfolder = os.path.dirname(
                                        database.replace(f"{root}/", "")
                                    )

                                    # Database header
                                    database_header = vcfdata_obj.get_header(
                                        database=database
                                    ).infos
                                    ...
                                    for info in list(database_header):
                                        ...
                                        # num
                                        if (
                                            database_header[info].num
                                            in GENOTYPE_MAP.keys()
                                        ):
                                            header_infos_dict[info]["Number"] = (
                                                GENOTYPE_MAP.get(
                                                    database_header[info].num
                                                )
                                            )
...
def databases_param(
    databases_infos_dict: dict,
    output: str = None,
    output_description: str = None,
    bcftools_preference: bool = False,
) -> dict:
    """
    ...
    for database_infos in databases_infos_dict:
        ...
        if bcftools_preference and database_format in BCFTOOLS_FORMAT:
            param_bcftools[database_infos] = databases_infos_dict[database_infos].get(
                "param"
            )
        else:
            param_parquet[database_infos] = databases_infos_dict[database_infos].get(
                "param"
            )

        # Stats
        param_stats["assembly"].append(
            databases_infos_dict[database_infos].get("assembly")
        )
        param_stats["databases"].append(database_infos)
        param_stats["releases"].append(
            databases_infos_dict[database_infos].get("release")
        )
        param_stats["formats"].append(
            databases_infos_dict[database_infos].get("format")
        )
        param_stats["fields"] += list(
            databases_infos_dict[database_infos].get("header").keys()
        )
    ...
    if param_parquet:
        param["annotation"]["parquet"] = {"annotations": param_parquet}

    # Add bcftools annotations to param
    if bcftools_preference and param_bcftools:
        param["annotation"]["bcftools"] = {"annotations": param_bcftools}
...
def databases_download_annovar(
    folder: str = None,
    files: list = None,
    ...
    for assembly in assemblies:
        ...
        if not os.path.exists(avdblist_folder_file):
            ...
            for line in lines:
                ...
                d = {"file": line[0], "version": line[1], "size": line[2]}
                data.append(d)
            ...
            for file in files_to_download:
                ...
                if files_in_folder:
                    ...
                    list_files_in_folder += [
                        os.path.basename(f) for f in files_in_folder
                    ]
                    for f in files_in_folder:
                        list_files_in_folder_size[os.path.basename(f)] = (
                            os.path.getsize(f)
                        )
            # List of files in folder
            log.debug(f"List of file in Annovar folder: {list_files_in_folder}")
            log.debug(
                f"List of file in Annovar folder with size: {list_files_in_folder_size}"
            )
            ...
            if files_to_download:
                ...
                for file in files_to_download:
                    log.info(
                        f"Download Annovar databases {[assembly]} - file '{file}' Downloading..."
                    )
...
def databases_download_snpeff(
    folder: str = None, assemblies: list = ["hg19"], config: dict = {}, threads: int = 1
) -> None:
    """
    ...
    for assembly in assemblies:
        ...
        if not os.path.exists(folder_assembly):
            ...
            if (
                ...
                log.debug(
                    f"snpEff databases downloading - list of databases: {snpeff_command_list_databases}"
                )
            ...
            with open(snpeff_databases_list_path, "r") as f:
                for line in f:
                    ...
                    snpeff_list_databases[cols[0]].append(
                        cols[-2].replace(",", "").replace("]", "").replace("[", "")
                    )
                    snpeff_list_databases[cols[0]].append(
                        cols[-1].replace(",", "").replace("]", "").replace("[", "")
                    )
...
def databases_download_genomes(
    assemblies: list,
...
def databases_download_refseq(
    assemblies: list,
    refseq_folder: str = None,
    refseq_url: str = None,
    refseq_prefix: str = "ncbiRefSeq",
    refseq_files: List = ["ncbiRefSeq.txt", "ncbiRefSeqLink.txt"],
    refseq_format_file: str = "ncbiRefSeq.txt",
    refseq_format_file_output: str = None,
    ...
    for assembly in assemblies:
        ...
        if os.path.exists(refseq_folder):
            ...
            existing_files = glob.glob(
                rf"{existing_files_path}/{refseq_prefix}*", recursive=True
            )
            ...
            for refseq_file in refseq_files:
                ...
                if f"{existing_files_path}/{refseq_file}" in existing_files:
                    log.info(
                        f"Download refSeq databases ['{assembly}'] - '{refseq_file}' already exists"
                    )
                ...
                # If refSeq file need to be downloaded
                    ...
                    try:
                        log.info(
                            f"Download refSeq databases ['{assembly}'] - '{refseq_file}' downloading..."
                        )
                        # Download file
                        download_file(file_url, file_path, threads=threads)
                        # Extract file
                    ...
                    # If fail, just pass to next url
                        ...
                        log.debug(
                            f"Download refSeq databases {assemblies} - '{assembly}' - '{refseq_file}' downloading failed"
                        )
                        raise ValueError(
                            f"Download refSeq databases {assemblies} - '{assembly}' - '{refseq_file}' downloading failed"
                        )
                ...
                file_path_bed_compressed_basename = os.path.basename(
                    file_path_bed_compressed
                )
                if refseq_file == refseq_format_file and (
                    new_refseq_file or not os.path.exists(file_path_bed)
                ):
                    log.info(
                        f"Download refSeq databases ['{assembly}'] - '{refseq_file}' formatting to '{file_path_bed_basename}'..."
                    )
                    # Format refSeq in BED format
                    databases_format_refseq(
                        refseq_file=file_path,
                        output_file=file_path_bed,
                    ...
                    # compress file
                    log.info(
                        f"Download refSeq databases ['{assembly}'] - '{file_path_bed_basename}' compressing to '{file_path_bed_compressed_basename}'..."
                    )
...
def databases_format_refseq(
            ...
            for l in fi:
                ...
                if not l.startswith("#"):
                    ...
                    condition = (
                        ...
                        (
                            include_non_canonical_chr
                            or re.match(r"^chr([0-9]*|X|Y)$", l[2])
                        )
                        ...
                        for i in range(len(exon_starts)):
                            ...
                            if include_utr_5 and include_utr_3:
                                ...
                                if int(exon_starts[i]) < int(l[6]):
                                    exons_to_write.append(
                                        [l[2], l[6], exon_ends[i], l[12], l[1], l[3]]
                                    )
                                else:
                                    exons_to_write.append(
                                        [
                                ...
                                if int(exon_ends[i]) > int(l[7]):
                                    exons_to_write.append(
                                        [l[2], exon_starts[i], l[7], l[12], l[1], l[3]]
                                    )
                                else:
                                    exons_to_write.append(
                                        [
                                ...
                                if int(exon_starts[i]) >= int(l[6]) and int(
                                    exon_ends[i]
                                ) <= int(l[7]):
                                    exons_to_write.append(
                                        [
                                ...
                                elif int(exon_starts[i]) < int(l[6]) and int(
                                    l[6]
                                ) <= int(exon_ends[i]) <= int(l[7]):
                                    exons_to_write.append(
                                        [l[2], l[6], exon_ends[i], l[12], l[1], l[3]]
                                    )
                                elif int(l[6]) <= int(exon_starts[i]) <= int(
                                    l[7]
                                ) and int(exon_ends[i]) > int(l[7]):
                                    exons_to_write.append(
                                        [l[2], exon_starts[i], l[7], l[12], l[1], l[3]]
                                    )
                                elif int(exon_starts[i]) <= int(l[6]) <= int(
                                    exon_ends[i]
                                ) and int(exon_starts[i]) <= int(l[7]) <= int(
                                    exon_ends[i]
                                ):
                                    exons_to_write.append(
                                        [l[2], l[6], l[7], l[12], l[1], l[3]]
                                    )
    ...
    if header:
        ...
        # Create header
        header_list = ["##fileformat=VCFv4.2", header_line]
...
def databases_download_dbnsfp(
    ...
    def get_columns_structure(
        ...
        if not columns_structure:
            log.error(
                f"Download dbNSFP {assemblies} - Check database structure - No database structure found"
            )
            raise ValueError(
                f"Download dbNSFP {assemblies} - Check database structure - No database structure found"
            )
            ...
            log.info(
                f"Download dbNSFP {assemblies} - Check database structure - Database structure generated"
            )
    ...
    def get_columns_select_clause(
        columns_structure: dict,
        assembly: str = "hg19",
        for_parquet: bool = False,
        sub_database: str = None,
        ...
        variant_position_columns = {
            "hg38": {
                "#chr": {"alias": "#CHROM", "type": "VARCHAR", "prefix": "chr"},
                "pos(1-based)": {"alias": "POS", "type": "BIGINT"},
            },
            "hg19": {
                "hg19_chr": {"alias": "#CHROM", "type": "VARCHAR", "prefix": "chr"},
                "hg19_pos(1-based)": {"alias": "POS", "type": "BIGINT"},
            },
            "hg18": {
                "hg18_chr": {"alias": "#CHROM", "type": "VARCHAR", "prefix": "chr"},
                "hg18_pos(1-based)": {"alias": "POS", "type": "BIGINT"},
            },
        }
        ...
        for column in columns_structure:
            ...
            # Keep assembly position columns
                ...
                column_prefix = (
                    variant_position_columns.get(assembly, {})
                    .get(column, {})
                    .get("prefix", "")
                )
                column_alias = (
                ...
                column_key = (
                    f""" {column_prefix_concat} "{column}" AS "{column_alias}" """
                )
                columns_select_position[column_key] = column_type
                # Force type on structure
                columns_structure[column] = (
                    variant_position_columns.get(assembly, {})
                    .get(column, {})
                    .get("type", None)
                )
                ...
                for other_assembly in variant_position_columns.keys():
                    if (
                        column
                        in variant_position_columns.get(other_assembly, {}).keys()
                    ):
                        # Force type on structure
                        column_type = (
                            variant_position_columns.get(other_assembly, {})
                            .get(column, {})
                            .get("type", None)
                        )
                ...
                if not column_to_remove:
                    ...
                    if (
                        sub_database is None
                        or sub_database in ["ALL"]
                        or column.upper() == sub_database
                        or column_alias.upper().startswith(f"{sub_database}_")
                    ):
                        if for_parquet:
                        ...
                        columns_where_annotations[column_where] = columns_structure[
                            column
                        ]
                        columns_list_annotations[column_alias] = columns_structure[
                            column
                        ]
        ...
        columns_select = (
            list(columns_select_position.keys())
            + list(columns_select_ref_alt.keys())
            + list(columns_select_info.keys())
            + list(columns_select_annotations.keys())
        )
        ...
        if print_log:
            log.debug(
                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Check database structure - {len(list(columns_select_annotations.keys()))} annotations found"""
            )
    ...
    def get_header(
        header_file: str,
        dbnsfp_readme_dest: str = None,
        columns_structure: dict = {},
        annotations: list = [],
        readme_annotations_description: dict = None,
    ) -> bool:
        """
        ...
        if not readme_annotations_description:
            readme_annotations_description = get_annotation_description(
                dbnsfp_readme_dest
            )
    ...
    for assembly in assemblies:
        ...
        database_files = get_database_files(
            f"{dbnsfp_zip_dest_folder}/dbNSFP{dbnsfp_release}_variant.chr*.gz"
        )
        log.debug(
            f"Download dbNSFP {assemblies} - Check database files - Found {len(database_files)} files"
        )
        if not database_files:
            log.error(
                f"Download dbNSFP {assemblies} - Check database files - No files found"
            )
            raise ValueError(
                f"Download dbNSFP {assemblies} - Check database files - No files found"
            )
        ...
        if columns_select_clause:
            ...
            for database_file in sorted(
                database_files, key=str.casefold, reverse=False
            ):

                log.info(
                    f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}'..."""
                )

                # Chromosome
                log.debug(
                    f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}' - Check chomosome..."""
                )
                query = f"""
                            SELECT "#chr"
                ...
                # Log
                log.debug(
                    f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}' - Write parquet files..."""
                )

                if os.path.exists(f"{output_parquet}"):
                    log.debug(
                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}' - Parquet file already exists"""
                    )
                    ...
                    if partition_size:
                        log.debug(
                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}' - Split parquet files..."""
                        )
                        for file in os.listdir(output_parquet):
                            ...
                            log.debug(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}' - Split parquet files ['{file}']..."""
                            )
                            if file_size > partition_size:
                                schema = pq.ParquetFile(file_path).schema_arrow
                                dd.read_parquet(file_path).repartition(
                                    partition_size=partition_size * 8
                                ).to_parquet(
                                    output_parquet,
                                    schema=schema,
                                    name_function=lambda x: f"{file_base}-{x}.parquet",
                                )
                                os.remove(file_path)
                            else:
                                new_file_name = os.path.join(
                                    output_parquet,
                                    file.replace(".parquet", "-0.parquet"),
                                )
                                os.rename(file_path, new_file_name)

                    # Log
                    log.debug(
                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Process file '{os.path.basename(database_file)}' - Write parquet file - done."""
                    )
            ...
            if not os.path.exists(header_file):
                ...
                if get_header(
                    ...
                    dbnsfp_readme_dest=dbnsfp_readme_dest,
                    columns_structure=columns_structure,
                    annotations=columns_annotations.keys(),
                    readme_annotations_description=readme_annotations_description,
                ):
                    log.debug(
                        f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Write header"
                    )
                else:
                    log.error(
                        f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Write header failed"
                    )
                    raise ValueError(
                        f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Write header failed"
                    )
    ...
    if generate_sub_databases:
        ...
        for assembly in assemblies:
            ...
            parquet_all_annotation = (
                f"{output_prefix}.ALL.partition.parquet/*/*.parquet"
            )
            ...
            for sub_database in sorted(
                set(sub_databases_structure), key=str.casefold, reverse=False
            ):
                ...
                # Parquet folder
                parquet_sub_database_annotation = (
                    f"{output_prefix}.{sub_database_name}.partition.parquet"
                )
                ...
                if columns_select_clause:
                    ...
                    if os.path.exists(f"{parquet_sub_database_annotation}"):
                        log.debug(
                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet folder already exists"""
                        )
                        parquet_partition_already_generated_list.append(sub_database)
                    else:
                        log.info(
                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet folder..."""
                        )
                        ...
                        for chromosome in chromosomes:
                            parquet_sub_database_annotation_chromosome = os.path.join(
                                parquet_sub_database_annotation, f"#CHROM={chromosome}"
                            )
                            Path(parquet_sub_database_annotation_chromosome).mkdir(
                            ...
                            if partition_size:
                                ...
                                for file in os.listdir(
                                    parquet_sub_database_annotation_chromosome
                                ):
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet folder ['{sub_database_name}'] generation - Split parquet files ['{file}']..."""
                                    )
                                    file_path = os.path.join(
                                        parquet_sub_database_annotation_chromosome, file
                                    )
                                    file_size = os.stat(file_path).st_size
                                    file_base = file.replace(".parquet", "")
                                    if file_size > partition_size:
                        ...
                        log.debug(
                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet folder ['{sub_database_name}'] generation - done."""
                        )
                    ...
                    if not os.path.exists(header_file):
                        ...
                        if get_header(
                            ...
                            log.debug(
                                f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet folder ['{sub_database_name}'] generation - Write header"
                            )
                        else:
                            log.error(
                                f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet folder ['{sub_database_name}'] generation - Write header"
    ...
    if generate_parquet_file or generate_vcf_file:
        ...
        for assembly in assemblies:
            ...
            for partition_parquet_folder in [partition_parquet_folder_all] + sorted(
                partition_parquet_folders, key=str.casefold, reverse=False
            ):
                ...
                input_partition_parquet = (
                    f"{output_prefix}.{sub_database}.partition.parquet"
                )
                input_partition_parquet_header = f"{input_partition_parquet}.hdr"

                # Input Parquet files
                input_parquet_files = sorted(
                    glob.glob(f"{input_partition_parquet}/*/*parquet"),
                ...
                if generate_parquet_file:
                    ...
                    if not os.path.exists(output_parquet):
                        ...
                        if input_parquet_files:
                            ...
                            # Log
                            log.info(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file..."""
                            )
                            ...
                            with pq.ParquetWriter(
                                ...
                                for parquet_file in input_parquet_files:
                                    ...
                                    if chromosome_folder != chromosome_folder_previous:
                                        ...
                                        log.debug(
                                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - Process '{os.path.basename(os.path.dirname(parquet_file))}/*' files..."""
                                        )
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - Process '{os.path.basename(os.path.dirname(parquet_file))}/{os.path.basename(parquet_file)}' file..."""
                                    )
                            ...
                            log.error(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - No parquet files found"""
                            )
                            raise ValueError(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - No parquet files found"""
                            )
                        ...
                        # Log
                        log.debug(
                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file already generated"""
                        )
                    ...
                    if not os.path.exists(output_header_file):
                        if os.path.exists(input_partition_parquet_header):
                            shutil.copy(
                                input_partition_parquet_header, output_header_file
                            )
                            log.debug(
                                f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - Write header"
                            )
                        else:
                            log.error(
                                f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - Write header failed"
                            )
                            raise ValueError(
                                f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - Parquet file - Write header failed"
                            )
                ...
                if generate_vcf_file:
                    ...
                    if not genomes_sizes:
                        genomes_sizes_file = os.path.join(
                            genomes_folder, assembly, f"{assembly}.fa.sizes"
                        )
                        if os.path.exists(genomes_sizes_file):
                            with open(genomes_sizes_file, "r") as f:
                                for line in f:
                                    genomes_sizes[line.split("\t")[0]] = line.split(
                                        "\t"
                                    )[1].strip()
                        else:
                            genomes_sizes = None
                    ...
                    if not os.path.exists(output_vcf):
                        ...
                        if input_parquet_files:
                            ...
                            # Log
                            log.info(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file..."""
                            )
                            ...
                            if column_info:
                                ...
                                with TemporaryDirectory(dir=output_assembly) as tmp_dir:
                                    ...
                                    # Log
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Check chomosomes and number of variants..."""
                                    )
                                    ...
                                    chromosomes = sorted(
                                        list(
                                            db_copy.query(query_chromosomes).df()[
                                                "chromosome"
                                            ]
                                        )
                                    )
                                    ...
                                    # Log
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Found {len(chromosomes)} chromosomes"""
                                    )
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Found {nb_variants} variants"""
                                    )

                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Generate files..."""
                                    )
                                    ...
                                    for parquet_file in input_parquet_files:
                                        ...
                                        query_nb_variants = f"""
                                                ...
                                                """
                                        parquet_file_nb_variants = sum(
                                            db_copy.query(query_nb_variants).df()[
                                                "nb_variants"
                                            ]
                                        )
                                        ...
                                        # Log
                                        log.debug(
                                            f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Process '{os.path.basename(os.path.dirname(parquet_file))}/{os.path.basename(parquet_file)}' file [{parquet_file_nb_variants} variants]..."""
                                        )

                                        # Query for parquet file
                                        file_num += 1
                                        vcf_file_gz = os.path.join(
                                            tmp_dir, f"variants.{file_num}.tsv.gz"
                                        )
                                        query_copy = f"""
                                                COPY (
                                    ...
                                    # Log
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - {len(list_for_vcf)} files process in parallel [{threads} threads]..."""
                                    )
                                    ...
                                    # Log
                                    log.debug(
                                        f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - {len(list_for_vcf)} files generated"""
                                    )
                                    ...
                                    header_file_tmp = os.path.join(
                                        tmp_dir, "header.hdr"
                                    )
                                    if os.path.exists(input_partition_parquet_header):
                                        shutil.copy(
                                            input_partition_parquet_header,
                                            header_file_tmp,
                                        )
                                        log.debug(
                                            f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Write header"
                                        )
                                    else:
                                        log.error(
                                            f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Write header failed"
                                        )
                                        raise ValueError(
                                            f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Write header failed"
                                        )
                                    ...
                                    for chromosome in chromosomes:
                                        header_list.append(
                                            f"##contig=<ID={chromosome},length={genomes_sizes.get(chromosome,0)},assembly={assembly}>"
                                        )
                                    ...
                                    # Log
                                    log.debug(
                                        f"Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - Concate and Compress files..."
                                    )
                                    concat_and_compress_files(
                                ...
                                log.error(
                                    f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - No INFO columns found"""
                                )
                                raise ValueError(
                                    f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - No INFO columns found"""
                                )
                            ...
                            log.error(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - No parquet files found"""
                            )
                            raise ValueError(
                                f"""Download dbNSFP ['{assembly}'] - Database '{sub_database}' - VCF file - No parquet files found"""
                            )
    ...
    if parquet_partition_generated_list:
        log.info(
            f"""Download dbNSFP ['{assembly}'] - {len(set(parquet_partition_generated_list))} Parquet folders generated {list(set(parquet_partition_generated_list))}"""
        )
    if parquet_partition_already_generated_list:
        log.info(
            f"""Download dbNSFP ['{assembly}'] - {len(set(parquet_partition_already_generated_list))} Parquet folders already generated {list(set(parquet_partition_already_generated_list))} """
        )
    if parquet_generated_list:
        log.info(
            f"""Download dbNSFP ['{assembly}'] - {len(set(parquet_generated_list))} Parquet files generated {list(set(parquet_generated_list))}"""
        )
    if parquet_already_generated_list:
        log.info(
            f"""Download dbNSFP ['{assembly}'] - {len(set(parquet_already_generated_list))} Parquet files already generated {list(set(parquet_already_generated_list))} """
        )
    if vcf_generated_list:
        log.info(
            f"""Download dbNSFP ['{assembly}'] - {len(set(vcf_generated_list))} VCF files generated {list(set(vcf_generated_list))}"""
        )
    if vcf_already_generated_list:
        log.info(
            f"""Download dbNSFP ['{assembly}'] - {len(set(vcf_already_generated_list))} VCF files already generated {list(set(vcf_already_generated_list))} """
        )
...
def databases_download_alphamissense(
    assemblies: list,
    alphamissense_folder: str = DEFAULT_ANNOTATIONS_FOLDER,
    alphamissense_url: str = DEFAULT_ALPHAMISSENSE_URL,
    threads: int = None,
) -> bool:
    """
    ...
    for assembly in assemblies:
        ...
        log.debug(
            f"{alphamissense_tsv}, {alphamissense_tsv_url}, {alphamissense_tsv_dest}, {alphamissense_tsv_dest_folder}"
        )

        # Download AlphaMissense
        if not os.path.exists(alphamissense_tsv_dest):
            log.info(
                f"Download AlphaMissense {assemblies} - Download '{alphamissense_tsv}'..."
            )
            download_file(
                alphamissense_tsv_url, alphamissense_tsv_dest, threads=threads
            )
        else:
            log.info(
                f"Download AlphaMissense {assemblies} - Database '{alphamissense_tsv}' already exists"
            )
        ...
        if os.path.exists(alphamissense_tsv_dest):
            ...
            log.error(
                f"Download AlphaMissense {assemblies} - Database '{alphamissense_tsv}' DOES NOT exists"
            )

    return True


def databases_download_exomiser(
    assemblies: list,
    exomiser_folder: str = DEFAULT_EXOMISER_FOLDER,
    exomiser_application_properties: str = None,
    exomiser_url: str = DEFAULT_EXOMISER_URL,
    exomiser_release: str = None,
    exomiser_phenotype_release: str = None,
    exomiser_remm_release: str = None,
    exomiser_remm_url: str = "https://kircherlab.bihealth.org/download/ReMM",
    exomiser_cadd_release: str = None,
    exomiser_cadd_url: str = "https://kircherlab.bihealth.org/download/CADD",
    exomiser_cadd_url_snv_file: str = "whole_genome_SNVs.tsv.gz",
    exomiser_cadd_url_indel_file: str = "InDels.tsv.gz",
    threads: int = 1,
) -> bool:
    """
    ...
    for assembly in assemblies:
        ...
        if not exomiser_release or exomiser_release.lower() in [
            ...
            exomiser_release_found = exomiser_application_properties_dict.get(
                f"exomiser.{assembly}.data-version", exomiser_release_default
            )
        else:
            exomiser_release_found = exomiser_release

        if not exomiser_phenotype_release or exomiser_phenotype_release.lower() in [
            "default",
            "auto",
            "config",
        ]:
            exomiser_phenotype_release_found = exomiser_application_properties_dict.get(
                f"exomiser.phenotype.data-version", exomiser_release_found
            )
        else:
        ...
        # Phenotype
        exomiser_phenotype_filename = (
            f"{exomiser_phenotype_release_found}_phenotype.zip"
        )
        exomiser_phenotype_filename_base = (
            f"{exomiser_phenotype_release_found}_phenotype"
        )
        exomiser_phenotype_file = os.path.join(
            exomiser_folder, exomiser_phenotype_filename
        )
        exomiser_phenotype_file_base = os.path.join(
            exomiser_folder, exomiser_phenotype_filename_base
        )
        exomiser_download_phenotype_url = os.path.join(
            exomiser_url, "data", exomiser_phenotype_filename
        )

        # Download Zip file
        if not os.path.exists(exomiser_phenotype_file):
            log.info(
                f"Download Exomiser {assemblies} - Download Phenotype '{exomiser_phenotype_filename}'..."
            )
            download_file(
                url=exomiser_download_phenotype_url,
                dest_file_path=exomiser_phenotype_file,
                threads=threads,
            )
        else:
            log.info(
                f"Download Exomiser {assemblies} - Database Phenotype '{exomiser_phenotype_filename}' already exists"
            )

        # Extract Zip file
        if not os.path.exists(exomiser_phenotype_file_base):
            log.info(
                f"Download Exomiser {assemblies} - Extract Phenotype '{exomiser_phenotype_filename}'..."
            )
            extract_file(file_path=exomiser_phenotype_file, path=None, threads=threads)
        else:
            log.info(
                f"Download Exomiser {assemblies} - Database Phenotype '{exomiser_phenotype_filename}' already extracted"
            )
        ...
        if not os.path.exists(exomiser_assembly_file):
            log.info(
                f"Download Exomiser {assemblies} - Download Data '{exomiser_assembly_filename}'..."
            )
            download_file(
                url=exomiser_download_assembly_url,
                dest_file_path=exomiser_assembly_file,
                threads=threads,
            )
        else:
            log.info(
                f"Download Exomiser {assemblies} - Database Data '{exomiser_assembly_filename}' already exists"
            )
        ...
        if not os.path.exists(
            ...
            log.info(
                f"Download Exomiser {assemblies} - Extract Data '{exomiser_assembly_filename}'..."
            )
            extract_file(
                file_path=exomiser_assembly_file,
                path=exomiser_assembly_folder,
                threads=threads,
            )
        else:
            log.info(
                f"Download Exomiser {assemblies} - Database Data '{exomiser_assembly_filename}' already extracted"
            )

        # Link Phenotype to assembly
        phenotype_assembly_link_src = f"../{exomiser_phenotype_filename_base}"
        phenotype_assembly_link_dest = os.path.join(
            exomiser_assembly_folder, exomiser_phenotype_filename_base
        )
        if not os.path.exists(phenotype_assembly_link_dest):
            log.info(
                f"Download Exomiser {assemblies} - Create Data '{os.path.basename(phenotype_assembly_link_dest)}' Phenotype link..."
            )

            os.symlink(phenotype_assembly_link_src, phenotype_assembly_link_dest)
        else:
            log.info(
                f"Download Exomiser {assemblies} - Database Data '{os.path.basename(phenotype_assembly_link_dest)}' Phenotype link already created"
            )

        # Generate properties for assembly
        exomiser_application_properties_assembly_file = os.path.join(
        ...
        if exomiser_remm_release:
            ...
            if exomiser_remm_release.lower() in ["default", "auto", "config"]:
                exomiser_remm_release_found = exomiser_application_properties_dict.get(
                    "remm.version", None
                )
        ...
        if exomiser_remm_release_found and exomiser_application_properties_dict.get(
            f"exomiser.{assembly}.remm-path", None
        ):
            exomiser_remm_download = True

        exomiser_assembly_remm_folder = os.path.join(exomiser_assembly_folder, "remm")
        exomiser_remm_path = os.path.join(
            exomiser_assembly_remm_folder,
            f"ReMM.v{exomiser_remm_release_found}.{assembly}.tsv.gz",
        )
        ...
        if exomiser_remm_download and not os.path.exists(exomiser_remm_path):
            ...
            log.info(
                f"Download Exomiser {assemblies} - Download REMM database '{os.path.basename(exomiser_remm_path)}'..."
            )
            exomiser_download_assembly_remm_url = os.path.join(
                exomiser_remm_url,
                f"ReMM.v{exomiser_remm_release_found}.{assembly}.tsv.gz",
            )
            exomiser_download_assembly_remm_tbi_url = os.path.join(
                exomiser_remm_url,
                f"ReMM.v{exomiser_remm_release_found}.{assembly}.tsv.gz.tbi",
            )
            exomiser_download_assembly_remm_md5_url = os.path.join(
                exomiser_remm_url, f"ReMM.v{exomiser_remm_release_found}.{assembly}.md5"
            )
            download_file(
                url=exomiser_download_assembly_remm_url,
                dest_file_path=exomiser_remm_path,
                threads=threads,
            )
            download_file(
                url=exomiser_download_assembly_remm_tbi_url,
                dest_file_path=exomiser_remm_path_tbi,
                threads=threads,
            )
            download_file(
                url=exomiser_download_assembly_remm_md5_url,
                dest_file_path=exomiser_remm_path_md5,
                threads=threads,
            )
        else:
        ...
        log.debug(
            f"{exomiser_remm_release_found} and {exomiser_remm_path} and os.path.exists({exomiser_remm_path})"
        )
        if (
            exomiser_remm_release_found
            and exomiser_remm_path
            and os.path.exists(exomiser_remm_path)
        ):
            log.info(
                f"Download Exomiser {assemblies} - Database REMM '{os.path.basename(exomiser_remm_path)}' already exists"
            )
            exomiser_application_properties_assembly["remm.version"] = (
                exomiser_remm_release_found
            )
            exomiser_application_properties_assembly[
                f"exomiser.{assembly}.remm-path"
            ] = "/".join(
                [
        ...
        if exomiser_cadd_release:
            if exomiser_cadd_release.lower() in ["default", "auto", "config"]:
                exomiser_cadd_release_found = exomiser_application_properties_dict.get(
                    "cadd.version", None
                )
        ...
        # Check if path is mandatory in application.properties
        if (
            exomiser_cadd_release_found
            and exomiser_application_properties_dict.get(
                f"exomiser.{assembly}.cadd-snv-path", None
            )
            and exomiser_application_properties_dict.get(
                f"exomiser.{assembly}.cadd-in-del-path", None
            )
        ):
            exomiser_cadd_download = True

        if exomiser_cadd_release_found:
            exomiser_assembly_cadd_folder = os.path.join(
                exomiser_assembly_folder, "cadd", exomiser_cadd_release_found
            )
            exomiser_cadd_snv_path = os.path.join(
                exomiser_assembly_cadd_folder, exomiser_cadd_url_snv_file
            )
            exomiser_cadd_indel_path = os.path.join(
                exomiser_assembly_cadd_folder, exomiser_cadd_url_indel_file
            )
        else:
        ...
        if exomiser_cadd_download and (
            not os.path.exists(exomiser_cadd_snv_path)
            or not os.path.exists(exomiser_cadd_indel_path)
        ):
            ...
            log.debug(
                f"Download Exomiser {assemblies} - Search for assembly '{assembly}' into gencode..."
            )
            ...
            if not os.path.exists(exomiser_cadd_snv_path):
                ...
                log.info(
                    f"Download Exomiser {assemblies} - Download CADD database '{os.path.basename(exomiser_cadd_snv_path)}'..."
                )
                exomiser_download_assembly_cadd_snv_url = os.path.join(
                    exomiser_cadd_url,
                    f"v{exomiser_cadd_release_found}",
                    genome_build_switch_to_gencode,
                    exomiser_cadd_url_snv_file,
                )
                exomiser_download_assembly_cadd_snv_tbi_url = os.path.join(
                    exomiser_cadd_url,
                    f"v{exomiser_cadd_release_found}",
                    genome_build_switch_to_gencode,
                    f"{exomiser_cadd_url_snv_file}.tbi",
                )
                download_file(
                    url=exomiser_download_assembly_cadd_snv_url,
                    dest_file_path=exomiser_cadd_snv_path,
                    threads=threads,
                )
                download_file(
                    url=exomiser_download_assembly_cadd_snv_tbi_url,
                    dest_file_path=exomiser_cadd_snv_path_tbi,
                    threads=threads,
                )
            ...
            if not os.path.exists(exomiser_cadd_indel_path):
                ...
                log.info(
                    f"Download Exomiser {assemblies} - Download CADD database '{os.path.basename(exomiser_cadd_indel_path)}'..."
                )
                exomiser_download_assembly_cadd_indel_url = os.path.join(
                    exomiser_cadd_url,
                    f"v{exomiser_cadd_release_found}",
                    genome_build_switch_to_gencode,
                    exomiser_cadd_url_indel_file,
                )
                exomiser_download_assembly_cadd_indel_tbi_url = os.path.join(
                    exomiser_cadd_url,
                    f"v{exomiser_cadd_release_found}",
                    genome_build_switch_to_gencode,
                    f"{exomiser_cadd_url_indel_file}.gz.tbi",
                )
                download_file(
                    url=exomiser_download_assembly_cadd_indel_url,
                    dest_file_path=exomiser_cadd_indel_path,
                    threads=threads,
                )
                download_file(
                    url=exomiser_download_assembly_cadd_indel_tbi_url,
                    dest_file_path=exomiser_cadd_indel_path_tbi,
                    threads=threads,
                )
        ...
        log.debug(
            f"{exomiser_cadd_release_found} and {exomiser_cadd_snv_path} and {exomiser_cadd_indel_path}"
        )
        if (
            exomiser_cadd_release_found
            and exomiser_cadd_snv_path
            and os.path.exists(exomiser_cadd_snv_path)
            and exomiser_cadd_indel_path
            and os.path.exists(exomiser_cadd_indel_path)
        ):
            log.info(
                f"Download Exomiser {assemblies} - Database CADD '{os.path.basename(exomiser_cadd_snv_path)}' already exists"
            )
            log.info(
                f"Download Exomiser {assemblies} - Database CADD '{os.path.basename(exomiser_cadd_indel_path)}' already exists"
            )
            exomiser_application_properties_assembly["cadd.version"] = (
                exomiser_cadd_release_found
            )
            exomiser_application_properties_assembly[
                f"exomiser.{assembly}.cadd-snv-path"
            ] = "/".join(
                [
                    "${exomiser.data-directory}",
                    "cadd",
            ...
            exomiser_application_properties_assembly[
                f"exomiser.{assembly}.cadd-in-del-path"
            ] = "/".join(
                [
                    "${exomiser.data-directory}",
                    "cadd",
...
def databases_download_dbsnp(
    assemblies: list,
    dbsnp_folder: str = DEFAULT_DBSNP_FOLDER,
    dbsnp_releases: list = ["b156"],
    dbsnp_release_default: str = None,
    dbsnp_url: str = DEFAULT_DBSNP_URL,
    dbsnp_url_files: dict = None,
    dbsnp_url_files_prefix: str = "GCF_000001405",
    ...
    for assembly in assemblies:
        ...
        for dbsnp_release in dbsnp_releases:
            ...
            # Folder
            dbsnp_folder_assembly_release = os.path.join(
                dbsnp_folder_assembly, dbsnp_release
            )
            ...
            if dbsnp_url_files and dbsnp_url_files.get(assembly, None):
                ...
                dbsnp_url_file = (
                    f"{dbsnp_url_files_prefix}.{dbsnp_assemblies_map.get(assembly)}.gz"
                )
            else:
                dbsnp_url_file = None

            if dbsnp_url_file:
                dbsnp_url_path = os.path.join(
                    dbsnp_url, dbsnp_release, "VCF", dbsnp_url_file
                )
            else:
            ...
            dbsnp_file = os.path.join(
                dbsnp_folder_assembly_release, os.path.basename(dbsnp_url_path)
            )
            ...
            if not os.path.exists(dbsnp_file):
                ...
                # Log
                log.info(
                    f"Download dbSNP {[assembly]} - Release {[dbsnp_release]} - Download '{dbsnp_url_file}'..."
                )
                ...
                # Download file
                download_file(
                    url=dbsnp_url_path, dest_file_path=dbsnp_file, threads=threads
                )
                ...
                download_file(
                    url=dbsnp_url_path_tbi,
                    dest_file_path=dbsnp_file_tbi,
                    threads=threads,
                )
                ...
                # Log
                log.info(
                    f"Download dbSNP {[assembly]} - Release {[dbsnp_release]} - Database {os.path.basename(dbsnp_file)} already downloaded"
                )
            ...
            dbsnp_vcf_file_buffer = os.path.join(
                dbsnp_folder_assembly_release, "dbsnp.buffer.vcf.gz"
            )

            # Construct File
            dbsnp_parquet_file = os.path.join(
                dbsnp_folder_assembly_release, "dbsnp.parquet"
            )
            dbsnp_parquet_file_buffer = os.path.join(
                dbsnp_folder_assembly_release, "dbsnp.buffer.parquet"
            )
            dbsnp_parquet_hdr_file = os.path.join(
                dbsnp_folder_assembly_release, "dbsnp.parquet.hdr"
            )
            ...
            if (
                os.path.exists(dbsnp_file)
                and (dbsnp_vcf and not os.path.exists(dbsnp_vcf_file))
                or (dbsnp_parquet and not os.path.exists(dbsnp_parquet_file))
            ):
                ...
                if dbsnp_vcf and not os.path.exists(dbsnp_vcf_file):
                    ...
                    write_vcf = False
                    if dbsnp_vcf:
                        dbsnp_files_already_generated.append(
                            os.path.basename(dbsnp_vcf_file)
                        )
                ...
                if dbsnp_parquet and not os.path.exists(dbsnp_parquet_file):
                    ...
                    if dbsnp_parquet:
                        dbsnp_files_already_generated.append(
                            os.path.basename(dbsnp_parquet_file)
                        )

                # Log
                if dbsnp_files_already_generated:
                    log.info(
                        f"Download dbSNP {[assembly]} - Release {[dbsnp_release]} - Files {dbsnp_files_already_generated} already generated"
                    )
                if dbsnp_files_to_generate:
                    log.info(
                        f"Download dbSNP {[assembly]} - Release {[dbsnp_release]} - Files {dbsnp_files_to_generate} generation..."
                    )
                ...
                genome_index = os.path.join(
                    genomes_folder, assembly, f"{assembly}.fa.fai"
                )
                ...
                db = Database(
                    database=dbsnp_file, format="vcf", conn_config=conn_config_db
                )
                ...
                db_header_new = db.get_header_from_list(
                    header_list=db_header_list_new
                    + db_header_list_contigs
                    + db_header_list_chrom
                )
                ...
                with pgzip.open(dbsnp_vcf_file_buffer, mode="w", thread=threads) as f:
                    ...
                    for df_chunk in chunk_csv:
                        ...
                        # Log
                        log.debug(
                            f"chunk {chunk_i} - {chunksize*chunk_i} lines processed"
                        )
        ...
        if dbsnp_release_default_found:
            ...
            dbsnp_release_default_link_dest = os.path.join(
                dbsnp_folder_assembly, "default"
            )
            if (
                not os.path.exists(dbsnp_release_default_link_dest)
                or dbsnp_release_default
            ):
                log.info(
                    f"Download dbSNP {[assembly]} - Release {[dbsnp_release_default_found]} - Defined as default"
                )
                if not os.path.exists(
                    dbsnp_release_default_link_dest
                ) or os.path.islink(dbsnp_release_default_link_dest):
                    remove_if_exists(dbsnp_release_default_link_dest)
                    os.symlink(
                        dbsnp_release_default_link_src, dbsnp_release_default_link_dest
                    )
                else:
                    log.warning(
                        f"Download dbSNP {[assembly]} - Default release path is not a symlink. Change not allowed"
                    )
            else:
                dbsnp_release_default_already = os.path.basename(
                    os.path.realpath(dbsnp_release_default_link_dest)
                )
                log.info(
                    f"Download dbSNP {[assembly]} - Release {[dbsnp_release_default_already]} - Already defined as default"
                )

    return True


def databases_download_hgmd(
    assemblies: list,
    hgmd_file: str,
    ...
    if os.path.exists(output_vcf):
        ...
        log.info(
            f"Convert HGMD database {[assembly]} - File '{os.path.basename(output_vcf)}' already exists"
        )

    else:

        log.info(
            f"Convert HGMD database {[assembly]} - File '{os.path.basename(hgmd_file)}' processing..."
        )
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            # Log
            log.debug(
                f"""Convert HGMD database {[assembly]} - Check chromosomes and number of variants..."""
            )
            ...
            query_chromosomes = f"""
                    ...
                    """
            chromosomes = sorted(
                list(db_copy.query(query_chromosomes).df()["chromosome"])
            )
            nb_variants = sum(db_copy.query(query_chromosomes).df()["nb_variants"])

            # Log
            log.debug(
                f"""Convert HGMD database {[assembly]} - Found {len(chromosomes)} chromosomes"""
            )
            log.debug(
                f"""Convert HGMD database {[assembly]} - Found {nb_variants} variants"""
            )
            ...
            # Append INFO MC
            header_list.append(
                '##INFO=<ID=MC,Number=1,Type=String,Description="HGMD Accession number">'
            )
            ...
            # Create HGMD VCF
            log.info(
                f"""Convert HGMD database {[assembly]} - Generate VCF file '{os.path.basename(output_vcf)}'"""
            )
            concat_and_compress_files(
            ...
            if to_parquet or to_tsv:
                output_parquet = os.path.join(
                    hgmd_folder_assembly, output_basename + ".parquet"
                )
                log.info(
                    f"""Convert HGMD database {[assembly]} - Generate Parquet file '{os.path.basename(output_parquet)}'"""
                )
                hgmd_database_to_parquet = Variants(
                    input=output_vcf,
            ...
            if to_tsv:
                output_tsv = os.path.join(
                    hgmd_folder_assembly, output_basename + ".tsv"
                )
                log.info(
                    f"""Convert HGMD database {[assembly]} - Generate TSV file '{os.path.basename(output_tsv)}'"""
                )
                hgmd_database_to_tsv = Variants(
