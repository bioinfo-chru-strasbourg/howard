...
def test_genomic_to_cdna_coord():
    ...
    for transcript_name, genomic_coord, cdna_coord_expected in _convert_coords:
        ...
        nose.tools.assert_equal(
            cdna_coord,
            cdna_coord_expected,
            repr((cdna_coord, cdna_coord_expected, transcript_name, genomic_coord)),
        )
...
def test_format_name():
    ...
    for expected_name, formatable, attrs in _parse_names:
        if formatable:
            ...
            nose.tools.assert_equal(name, expected_name, (name, expected_name, attrs))
...
def test_format_prefix():
    ...
    result = obj.format_prefix(
        use_gene=True, use_exon=True, use_protein=True, full_format=True
    )
    ...
    result = obj.format_prefix(
        use_gene=True, use_exon=False, use_protein=False, full_format=False
    )
...
def test_name_to_variant():
    ...
    genome = MockGenomeTestFile(
        db_filename="hg19.fa",
        filename=tests_folder + "/data/test_name_to_variant.genome",
        create_data=False,
    )
    ...
    for hgvs_name, variant, name_canonical, var_canonical in _name_variants:
        if var_canonical:
            ...
            nose.tools.assert_equal(
                hgvs_variant, variant, repr([hgvs_name, variant, hgvs_variant])
            )
...
def test_matches_ref():
    ...
    for hgvs_string, expected_result in expected:
        ...
        nose.tools.assert_equal(
            matches_ref, expected_result, hgvs_string + " matches reference base"
        )
...
_parse_names = [
    ...
    #
    (
        "NC_000010.10:g.89623915_89623920=",
        False,
        {
            "chrom": "NC_000010.10",
        },
...
_name_variants = [
    ...
    # Same as above but without optional trailing base
    ("NM_000492.3:c.1155_1156dup", ("chr7", 117182104, "A", "AAT"), False, True),
    ...
    # Genomic indels.
    ("chr17:g.7126029_7126030insTG", ("chr17", 7126028, "A", "AGT"), True, True),
    (
    ...
    # Genomic dup
    ("chr7:g.117182108_117182109dup", ("chr7", 117182104, "A", "AAT"), False, True),
    ("chr7:g.117182105_117182106dupAT", ("chr7", 117182104, "A", "AAT"), False, True),
