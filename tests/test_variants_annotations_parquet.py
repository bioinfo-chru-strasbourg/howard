...
def test_annotation_parquet_fast():
    """
    This function tests the annotation of variants using parquet and fast strategy.
    """
    ...
    with TemporaryDirectory(dir=tests_folder) as tmp_dir:
        ...
        # Load output file
        variants_output = Variants(conn=None, input=output_vcf, load=True)

        # query annotated variant
        result = variants_output.get_query_to_df(
            "SELECT 1 AS count FROM variants WHERE \"#CHROM\" = 'chr7' AND POS = 55249063 AND REF = 'G' AND ALT = 'A' AND INFO = 'DP=125;nci60=0.66'"
        )
        assert len(result) == 1
